...
if ($LoggedInUsernames.Count -eq 1 -and $DisconnectedUsers.Count -eq 0) {
    Write-Output "You are the only logged-in user, and there are no disconnected sessions. Continuing..."
}
else {
    Write-Output "There are other active, disconnected sessions or unauthorised users logged in. Please sign out these users before continuing." -ForegroundColor Red
    return
}
